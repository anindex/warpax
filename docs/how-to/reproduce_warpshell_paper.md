...
| **T-shell vorticity $\to$ Type-IV** | Type-I in bulk, Type-IV in the low-density outer edge ($r\ge R_2$) for $v_0>0$; imag. eigenvalue scale linear in $v_0$ (outer-edge log-log slope $1.01\pm0.01$, $=0$ at $v_0=0$); standard + generalized-pencil + 50-digit gate all Type-IV | `scripts/run_tshell_typeIV_onset.py` |
...
print(float(anec.symplectic.line_integral), float(anec.symplectic.max_abs_g_kk))  # +4.5e-3, ~5e-5
```
