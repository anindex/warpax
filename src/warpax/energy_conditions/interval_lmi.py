...
def _demo() -> None:
    ...
    from .enclosure import alcubierre_metric, van_den_broeck_metric
    ...
    # The regression pin for the whole design. Van den Broeck on the axis satisfies
    # the null, weak and dominant conditions and violates only the strong one --
    # which is invisible to a null witness, because Theta(k,k) = T(k,k) there. If
    # the SEC witness ever reverts to the sphere this reads "inconclusive".
    ...
    # certified strictly positive. "inconclusive" is the correct answer, and the
    # bracket must show it is a saturation refusal rather than a failure. Pinned on
    # Alcubierre only: a construction with a 1/r tail is genuinely decidable here.
    ...
    print(f"interval_lmi demo OK: wall NEC in "
          f"[{wall['nec_lower']:.6f}, {wall['nec_upper']:.6f}] -> {wall['nec']}, "
          f"SEC -> {wall['sec']}, DEC -> {wall['dec']}; "
          f"VdB axis SEC -> {axis['sec']}; exterior NEC -> {far['nec']}")
