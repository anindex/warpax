...
relative to the float64 components they are handed. Running the interval curvature
chain and the interval LDL^T multiplier test at chosen points makes the verdict a
statement about the spacetime rather than about a snapshot of its stress-energy.

A handful of points per drive, not a grid: the interval chain costs seconds per
point, and the claim it supports is qualitative, that the type-free decision survives
being taken back to the metric.
