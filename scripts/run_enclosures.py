...
def write_table(rows, out_path):
    lines = [
        ...
        r"  \toprule",
        r"  Metric & $\mathcal N(x_*)$ & $\mathcal N_{\rm lower}$"
        r" & $\mathcal N_{\rm upper}$ & local interval width \\",
        r"  \midrule",
    ]
    for name in ORDER:
        if name not in rows:
        ...
        lo, hi = row["point_lower"], row["point_upper"]
        if not all(math.isfinite(x) for x in (lo, hi, row["lower"], row["upper"])):
            raise ValueError(f"{name}: cannot publish a nonfinite enclosure")
        if not row["lower"] <= lo <= hi or not row["lower"] <= row["upper"]:
            raise ValueError(f"{name}: inconsistent point/global enclosure")
        point_value = (lo + hi) / 2
        # Widen the printed global enclosure when rounding to six decimals.
        unit = Decimal("0.000001")
        lower = Decimal.from_float(row["lower"]).quantize(unit, rounding=ROUND_FLOOR)
        upper = Decimal.from_float(row["upper"]).quantize(unit, rounding=ROUND_CEILING)
        lines.append(
            f"  {label} & ${point_value:.6f}$ & ${lower:.6f}$ & ${upper:.6f}$"
            f" & ${hi - lo:.2e}$ \\\\"
        )
...
def certify_one(job):
    ...
    point = certify_point_from_metric(metric, (0.0, enc.argmin[0], enc.argmin[1], 0.0), prec=80)
    return name, {
        "lower": enc.lower,
        "upper": enc.upper,
        "width": enc.width,
        "point_lower": point["nec_lower"],
        "point_upper": point["nec_upper"],
        "point_interval_precision_bits": 80,
...
def main():
    ...
    for name in ORDER:
        ...
        print(
            ...
            f"tail_excluded={r['tail_excluded']}",
            flush=True,
        )
