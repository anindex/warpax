...
def segre31_block(f, r=0.7, p=0.3):
    """Segre [3,1]: a size-three Jordan chain for every ``f != 0``."""
    return np.array(
        [
            [r, 0.0, -f, 0.0],
            [0.0, -r, f, 0.0],
...
def type_iii_arm(fs, directions):
    """Where does the float64 label lose a true Type III, and does the LMI care?"""
...
def main():
    ...
    path = RESULTS / "type_transitions.json"
